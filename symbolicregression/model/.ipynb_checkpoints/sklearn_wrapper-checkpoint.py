...
class SymbolicTransformerRegressor(BaseEstimator):
    ...
    def fit(self, X, Y, required_operators=None, verbose=False):
        env = self.env
        ...
        for seq_id in range(len(scaled_X)):
            if len(scaled_X[seq_id][0]) > env.generator.max_input_dimension:
                return self
            ...
            for chunk in chunks(np.arange(B), int(10000 / T)):
                ...
                if generate_beam:
                    _, _, generations = decoder.generate_beam(
                        ...
                        group_ids=torch.arange(len(x_ids_chunk))
                    ) 
                ...
                if verbose:
                    print("finished forward of chunk:{}".format(chunk))
        ...
        for i in range(bs):
            ...
            if verbose: 
                print("start evaluation of candidates")
            ...
            for refinement_type in refinements_types:
                ...
                expand = "expand" in refinement_args and refinement_args["expand"] == "True"
                if expand:
            ...
            if verbose:
                for gen in gens:
                    print(gen["refinement_type"], gen["predicted_tree"])
...
if __name__ == "__main__":
    ...
    predicted_tree = est.fit([X_to_fit, X_to_fit], [Y_to_fit, Y_to_fit], verbose=True, required_operators=None, max_input_points=10000)
